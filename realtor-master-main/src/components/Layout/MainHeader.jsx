...
const MainHeader = () => {
  ...
  const btnContent = isAuthenticated ? "На вихід" : "Авторизація";
  ...
  const mobileMenu = (
    <div className="w-11/12 max-w-2xl mt-4 lg:hidden">
      ...
      <div className="flex justify-between py-3 pl-6">
        ...
        >
          {!isAuthenticated ? <Link to="/login">{btnContent}</Link> : "На вихід"}
        </button>
        <button
      ...
      <nav className="flex lg:flex items-center justify-between px-6  font-Poppins">
        ...
        <ul className=" hidden lg:flex px-6 text-ash  ">
          ...
          {isAuthenticated && role === 'realtor'
            &&
            <li>
              <NavLink to="/panel" activeClassName="text-blue" className="px-4 ">
                Керування
              </NavLink>
            </li>
          }
