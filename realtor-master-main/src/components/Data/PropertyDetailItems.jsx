...
const PropertyDetailItems = ({
  ...
  const [currRieltor, setcurrRieltor] = useState(null);
  const rid = realtor_id;
  const isAuthenticated = useSelector((state) => state.auth.isAuthenticated);
  console.log(currRieltor);
  useEffect(() => {

    const getReiltor = async () => {
      try {
        const data = await fetch(`http://localhost:8000/api/realtors/${rid}`);
        const dataJson = await data.json()
        setcurrRieltor(dataJson);
      } catch (error) {
